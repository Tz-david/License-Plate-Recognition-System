...
def sql(TIME, COLOR1, TEXT1, COLOR2, TEXT2, API, SOURCE):
    try:
        # 打开数据库连接
        db = pymysql.connect(host="localhost", port=3306, user="root", password=",=6_t2G:i5(h", database="plate_ocr")
...
def create_signsql():
    try:
        # 打开数据库连接
        # db = pymysql.connect("localhost", "root", ",=6_t2G:i5(h", "OCR")
        db = pymysql.connect(host="localhost", port=3306, user="root", password=",=6_t2G:i5(h", database="plate_ocr")
